...
function writeSessionRule(wtPath: string, ctx: SessionContext): void {
  const rulesDir = resolve(wtPath, ".claude", "rules");
  if (!existsSync(rulesDir)) mkdirSync(rulesDir, { recursive: true });
  const lines = [
    "# Session Context (auto-generated by claudet)",
    "",
    `Read the plan file at ${ctx.planPath} at session start.`,
    "",
    "## Worktree",
    `- **Branch:** ${ctx.branch}`,
    `- **Target:** ${ctx.target}`,
  ...
  if (ctx.lastProgress) {
    lines.push(`- **Last progress:** ${ctx.lastProgress}`);
  }
  if (ctx.pr) {
    lines.push(
      "",
  ...
  lines.push("");
  writeFileSync(resolve(rulesDir, "session.md"), lines.join("\n"));
}

function launchClaude(cwd: string, ctx?: SessionContext): void {
  if (ctx) {
    writeSessionRule(cwd, ctx);
  }
...
function installCommand(): void {
  ...
  // 2. Update statusline in settings.json
  const settings = tryParseJson<Record<string, unknown>>(
    tryReadFileSync(GLOBAL_SETTINGS_FILE),
    {},
  );
